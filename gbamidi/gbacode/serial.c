...
void serialInit() {
	// Stage2 loading uses normal 32-bit SIO, so do not inherit SIOCNT state.
	// The SpritesMods MIDI runtime receives 16-bit link words via multiplayer
	// data register 0 and wants a communication interrupt on completion.
	REG_R=0;
